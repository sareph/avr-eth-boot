...
typedef void (*link_status_callback)(uint8_t link_up, uint8_t flags);

#define ENC28J60_LINK_FLAG_FULLDUPLEX 		0x01
#define ENC28J60_LINK_FLAG_POLARITY_REV 	0x02
...
void enc28j60_crc_tx_add(uint16_t start, uint16_t len, uint16_t pos);
#endif
void enc28j60_init_clock(void);
...
uint16_t enc28j60_receive_packet(uint8_t * buff,uint16_t max_len, uint8_t *pkts);
uint8_t	enc28j60_get_revision(void);
uint8_t enc28j60_have_packet(void);
